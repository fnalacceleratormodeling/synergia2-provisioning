...
#  -DCMAKE_CXX_FLAGS="-arch=sm_80"
#
# otherwise it should be automatically detected.

# external kokkos usage runs into issues
# with linking MPI libraries at build time
# after successfull configure, turn it off
# until the reason behind the issue is 
# investigated and fixed

CC=cc CXX=CC \
cmake -DCMAKE_INSTALL_PREFIX=${SYNINSTALL} \
  -DCMAKE_BUILD_TYPE=Release \
  -DSRUN_MPI_PMIX=cray_shasta \
