...
then
    ...
    if cmake -DBUILD_PYTHON_BINDINGS=1 -DCHEF_DIR=${SYNINSTALL}/lib/chef/cmake  -DFFTW3_INCLUDE_DIR=${BOOST_ROOT}/include -DFFTW3_LIBRARIES=${BOOST_ROOT}/lib/libfftw3.so -DFFTW3_MPI_LIBRARIES=${BOOST_ROOT}/lib/libfftw3_mpi.so -DFFTW3_OMP_LIBRARIES=${BOOST_ROOT}/lib/libfftw3_omp.so -DUSE_SIMPLE_TIMER=0 -DUSE_PYTHON_3=1 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX:PATH=${SYNINSTALL} -DPYTHON_EXECUTABLE=${PY_EXE} ${SYNSRC} |& tee synergia2.cmake.out
    then
    ...
    if make -j 8 |& tee synergia2.make.out
    then
