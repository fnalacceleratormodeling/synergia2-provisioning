...
CC=gcc CXX=g++ \
scl enable gcc-toolset-11 "cmake -DCMAKE_INSTALL_PREFIX=${SYNINSTALL} \
  ...
  ${SYNSRC}" |& tee synergia2.cmake.out
    ...
    scl enable gcc-toolset-11 "make VERBOSE=t"  |& tee synergia2.make.out
    if [ $? -eq 0 ]
    then
    ...
    if  exl enable gcc-toolset-11 "make install" |& tee synergia2.install.out
    then
