...
# put install and cuda directory in paths
if echo ${PATH} | grep -qv syn2-dev
then
    PATH=${SYNINSTALL}/bin:/usr/local/cuda/bin:$PATH
fi
...
then
    ...
    then
        export LD_LIBRARY_PATH=${SYNINSTALL}/lib:/usr/local/cuda/lib64
    else
        export LD_LIBRARY_PATH=${SYNINSTALL}/lib:/usr/local/cuda/lib64:$LD_LIBRARY_PATH
    fi
fi
