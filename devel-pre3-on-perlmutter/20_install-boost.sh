...
then
    ...
    BOOSTURL="https://archives.boost.io/release/1.76.0/source/boost_1_76_0.tar.bz2"
    ...
    CC=cc CXX=CC ./bootstrap.sh --prefix=${SYNINSTALL} --with-python=${PY_EXE}  --with-libraries=python,regex,filesystem,system,test,serialization,iostreams |& tee bootstrap.out
    ...
    ./b2 cxxflags="-std=c++14" --build-dir=${BOOSTBLD} install |& tee b2.out
