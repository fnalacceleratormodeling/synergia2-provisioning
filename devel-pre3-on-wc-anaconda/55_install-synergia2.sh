...
then
    ...
    if cmake -DBUILD_PYTHON_BINDINGS=1 -DCHEF_DIR=${SYNINSTALL}/lib/chef/cmake -DBOOST_ROOT=${SYNINSTALL} -DGSL_ROOT_DIR=${GSL_DIR} -DFFTW3_INCLUDE_DIR=${FFTW_INC} -DFFTW3_LIBRARIES=${FFTW_DIR}/lib/libfftw3.so -DFFTW3_MPI_LIBRARIES=${FFTW_DIR}/lib/libfftw3_mpi.so -DFFTW3_OMP_LIBRARIES=${FFTW_DIR}/lib/libfftw3_omp.so -DHDF5_ROOT=${HDF5_DIR} -DEIGEN3_INCLUDE_DIR=${SYNINSTALL}/include/eigen3 -DUSE_SIMPLE_TIMER=0 -DUSE_PYTHON_3=1 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX:PATH=${SYNINSTALL} -DPYTHON_EXECUTABLE=${ANACONDA_ROOT}/bin/python3 ${SYNSRC} |& tee synergia2.cmake.out
    then
