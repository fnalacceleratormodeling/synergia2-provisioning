...
then
    ...
    scl enable devtoolset-8 'cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="${CHEF_INSTALL_DIR}"  -DBOOST_ROOT=${SYNINSTALL}  -DBUILD_PARSER_MODULES=0 -DBUILD_PYTHON_BINDINGS=1 -DBUILD_SHARED_LIBS=1 -DUSE_PYTHON_3=1  ${CHEFSRC}' |& tee chef.cmake.out
    ...
    scl enable devtoolset-8 'make -j 2 VERBOSE=true ' |& tee chef.make.out
    if [ $? -ne 0 ]
    then
